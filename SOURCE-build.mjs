...
<div class="window"><div class="track">${[0,1].map(copy=>`<div class="set"${copy?` aria-hidden="true"`:''}>${partners.map(([name,id])=>`<div class="logo" title="${name}"><img src="${u('assets/partners/partner-'+id+'.png')}" alt="${copy?'':name}" loading="eager" decoding="async"></div>`).join('')}</div>`).join('')}</div></div></div></section>
...
<section class="container about-img"><img src="${u('assets/images/site-6.webp')}" alt="RCC construction site and execution teams"><span class="mono">ON-SITE EXECUTION / GATI SITE GALLERY</span></section>
